...
Now that the apprentice has learned a fast imitation of the expert (on the moves seen so far), that intuition can be of use to the expert. When the expert now wishes to make a move, a small set of candidate moves are suggested very quickly by the apprentice which the expert can then consider in depth, possibly also guided during this slow thought process by other quick insights from the apprentice.
...
At the Expert Improvement phase we use the apprentice to direct the Monte Carlo Tree Search algorithm toward promising moves, effectively reducing the game tree search breadth. In this way, we bootstrap the knowledge acquired by Imitation Learning back into the planning algorithm.
...
$$
UCT(s,a) + w_a \frac{\hat{\pi}(a|s)}{n(s,a)+1} + w_v\hat{Q}(s,a)
\tag{1}\label{eq:uct} + net value term
$$

Here $$s$$ is the state of the Hex board, $$a$$ is a possible action (ie move) from $$s$$. The term $$UCT(s,a)$$ represents the classical Upper Confidence Tree formula[^MCTS] used in MCTS, $$\hat{\pi}$$ is the apprentice’s policy, and $$\hat{Q}(s,a)$$ is the average of network value estimates for simulations through the edge $$s, a$$. $$n(s,a)$$ the number of visits currently made by the search algorithm through state $$s$$ and taking action $$a$$; $$w_a$$ and $$w_v$$ are empirically chosen weighting factors that balance the slow thinking of the expert with the fast intuition of the apprentice.

Through the additional terms, the neural network apprentice guides the search to more promising moves, and rejects weak moves more quickly.


To generate the data for training the apprentice (during each Imitation Learning phase), a batch approach generates data afresh, discarding all data from previous iterations. We also consider an online version in which we instead keep a running buffer of the most recent moves generated, and an exponentially weighted online version that uses more moves from more recent experts (which correspond to the strongest, most recent play).  A comparison of these different approaches is given below, in which we compare the strength (measured in terms of the [ELO](https://en.wikipedia.org/wiki/Elo_rating_system) score) of the learned algorithm against a measure of training time.  
